<template>
  <div class="main-content">
      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '商品类型' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search"
                      v-model="searchForm.shangpinleixing" placeholder="商品类型" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
                      v-model="searchForm.shangpinleixing" placeholder="商品类型" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.shangpinleixing" placeholder="商品类型"
                      clearable></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
                v-if="isAuth('paimaishangpin','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                type="success"
                icon="el-icon-plus"
            ...
            <el-button
                v-if="isAuth('paimaishangpin','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
                type="success"
                @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1 ? '新增' : '' }}<i class="el-icon-plus el-icon--right"/></el-button>
            <el-button
                v-if="isAuth('paimaishangpin','新增') && contents.btnAdAllIcon == 0"
                type="success"
                @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1 ? '新增' : '' }}
            </el-button>
            <el-button
                v-if="isAuth('paimaishangpin','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
            ...
            <el-button
                v-if="isAuth('paimaishangpin','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
                :disabled="dataListSelections.length <= 0"
                type="danger"
                @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1 ? '删除' : '' }}<i class="el-icon-delete el-icon--right"/></el-button>
            <el-button
                v-if="isAuth('paimaishangpin','删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
                :disabled="dataListSelections.length <= 0"
                type="danger"
                @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1 ? '删除' : '' }}
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              <el-button
                  v-if="isAuth('paimaishangpin','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">
                {{ contents.tableBtnFont == 1 ? '详情' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('paimaishangpin','查看') && contents.tableBtnIcon == 0" type="success" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1 ? '详情' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','竞拍') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="success" icon="el-icon-tickets" size="mini"
                  @click="lishijingpaiCrossAddOrUpdateHandler(scope.row,'cross')">
                {{ contents.tableBtnFont == 1 ? '竞拍' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','竞拍') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  type="success" size="mini" @click="lishijingpaiCrossAddOrUpdateHandler(scope.row,'cross')">
                {{ contents.tableBtnFont == 1 ? '竞拍' : '' }}<i class="el-icon-tickets el-icon--right"/></el-button>
              <el-button v-if="isAuth('paimaishangpin','竞拍') && contents.tableBtnIcon == 0" type="success" size="mini"
                         @click="lishijingpaiCrossAddOrUpdateHandler(scope.row,'cross')">
                {{ contents.tableBtnFont == 1 ? '竞拍' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '修改' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('paimaishangpin','修改') && contents.tableBtnIcon == 0" type="primary" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? '修改' : '' }}
              </el-button>


              <el-button
                  v-if="isAuth('paimaishangpin','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                  type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">
                {{ contents.tableBtnFont == 1 ? '删除' : '' }}
              </el-button>
              <el-button
                  v-if="isAuth('paimaishangpin','删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('paimaishangpin','删除') && contents.tableBtnIcon == 0" type="danger" size="mini"
    ...
    <lishijingpai-cross-add-or-update v-if="lishijingpaiCrossAddOrUpdateFlag" :parent="this"
                                      ref="lishijingpaiCrossaddOrUpdate"></lishijingpai-cross-add-or-update>
...
import lishijingpaiCrossAddOrUpdate from "../lishijingpai/add-or-update";
...
export default {
  data() {
    return {
      ...
      lishijingpaiCrossAddOrUpdateFlag: false,
      contents: {
  ...
  methods: {
    ...
    lishijingpaiCrossAddOrUpdateHandler(row, type) {
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.lishijingpaiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj', row);
      this.$storage.set('crossTable', 'paimaishangpin');
      this.$nextTick(() => {
        this.$refs.lishijingpaiCrossaddOrUpdate.init(row.id, type);
      });
    },
    init() {
      this.$http({
